...
TimelineDlg::TimelineDlg(wxWindow* parent, Timeline* timeline) : mTimeline(timeline)
{
	...
	mMachineStartTime = timeline->GetMachineStartTime();
	...
	auto startTimeCtrl = XRCCTRL(*this, "StartTime", wxTextCtrl);
	wxIntegerValidator<int> startTimeValidator(&mMachineStartTime);
...
void TimelineDlg::OnOK(wxCommandEvent& event)
{
    if ( Validate() && TransferDataFromWindow() )
    {
		...
		mTimeline->SetMachineStartTime(mMachineStartTime);
