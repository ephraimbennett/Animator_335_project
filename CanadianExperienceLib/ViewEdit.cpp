...
ViewEdit::ViewEdit(wxFrame* parent) :wxScrolledCanvas(parent, wxID_ANY)
{
	...
	parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnEditMachineProperties, this, XRCID("EditMachineProperties"));
}
...
/**
 * Handler for updating the machine properties
 * @param event
 */
void ViewEdit::OnEditMachineProperties(wxCommandEvent &event)
{
	for (auto actor : *GetPicture())
	{
		if (actor->GetName() == L"machine_system")//if this is the machine actor
		{
			actor->GetRoot()->ShowDialog(this->GetParent());
		}
	}
}
